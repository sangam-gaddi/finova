...
// Mock pkg properties as they are missing in NextJS root package.json
const pkgMeta = {
    version: '0.8.5',
    name: 'bec-vortex-os',
...
export function BootSequence({ onComplete }: BootSequenceProps) {
    ...
    useEffect(() => {
        ...
        const generateLogs = async (): Promise<LogEntry[]> => {
            ...
            add('kernel', `${pkgMeta.build.productName} version ${pkgMeta.version}-generic (build@${pkgMeta.name})`);
            add('kernel', `Command line: BOOT_IMAGE=/boot/vmlinuz-${pkgMeta.version} root=UUID=7427-42A9 ro quiet splash`);
