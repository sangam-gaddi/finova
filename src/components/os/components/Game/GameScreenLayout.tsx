...
import { validateIntegrity } from "@/components/os/utils/integrity";
import { ConnectivityBadge } from "@/components/os/components/ui/ConnectivityBadge";
import { useI18n } from "@/components/os/i18n/index";
import { useAppContext } from "@/components/os/components/AppContext";
const background = '/os-assets/images/wallpaper-nebula.avif'; // Restored legacy background
                ...
                <div className={`shrink-0 text-center flex flex-col gap-2 items-center w-full max-w-lg relative z-10`}>
                    <div className={`flex flex-col justify-center items-center gap-2 ${mode === 'glass' ? 'text-xs font-mono' : 'w-full text-[10px] uppercase font-mono text-white/30 tracking-widest'}`}>
                        ...
                        <div className={`flex items-center justify-center ${mode === 'glass' ? 'gap-2 md:gap-4 text-[10px] md:text-xs font-mono text-white/50' : 'gap-2 text-white/20'}`}>
                            {footerActions || (
                                <>
                                    <span>©2026 <a href={"https://github.com/bec-team/bec-vortex-os.js"} target="_blank" rel="noreferrer" className="hover:text-(--accent-user) decoration-white/30 hover:decoration-(--accent-user)">BEC VORTEX OS 0.8.5</a> // <a href="https://instagram.com/bec-team" target="_blank" rel="noreferrer" className="hover:text-(--accent-user) decoration-white/30 hover:decoration-(--accent-user)">BEC VORTEX OS</a> // <a href="https://facebook.com/dopepxls" target="_blank" rel="noreferrer" className="hover:text-(--accent-user) decoration-white/30 hover:decoration-(--accent-user)">BEC Team</a><br />
                                        Open-source (<a href="https://github.com/bec-team/bec-vortex-os.js/blob/main/LICENSE" target="_blank" rel="noreferrer" className="hover:text-(--accent-user) decoration-white/30 hover:decoration-(--accent-user)">AGPL-3.0</a>) · All rights reserved beyond license terms</span>
                                </>
                            )}
                        </div>
                    </div>
